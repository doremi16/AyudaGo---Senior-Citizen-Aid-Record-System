...
async def login(
    ...
    admin = db.query(Admin).filter_by(admin_email=username, admin_password=password).all()
    if admin: 
...
#             {"request": request,  "bar_chart_html": bar_chart_html, "pie_chart_html": pie_chart_html, "user_count_chart_html":user_count_chart_html}
#         )
...
@app.post("/userhome")
async def userhome(request: Request):
...
async def fundstatus(request: Request, claim_status=HTMLResponse):
    if claim_status==True:
       return templates.TemplateResponse("user/statpending.html", {"request": request, "claim_status": [claim_status]})
    else:
        return templates.TemplateResponse("user/statpending.html", {"request": request})

@app.get("/fundstatus_c")
async def fundstatus_c(request: Request, claim_status=HTMLResponse):
    if claim_status==True:
       return templates.TemplateResponse("user/statclaimed.html", {"request": request, "claim_status": [claim_status]})
    else:
       return templates.TemplateResponse("user/statclaimed.html", {"request": request})
    
@app.get("/proceed")
def proceed():
    return RedirectResponse(url="/fundstatus_c")   
